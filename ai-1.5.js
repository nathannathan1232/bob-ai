...
function messageIn(e){
	...
	if(shouldRespond(msg))			// Return a message or false
		...
		//if(last_message)
			//learnResponse(last_message, msg_words[0].split(" "));
		last_message = sentences[0].split(" ");
		return false;
	}
}
...
/*
	Learns how to respond.
*/

function learnResponse(msg_a, msg_b){
	var main_a = findMainWords(a);

	for(var a = 0; a < main_a.length; a++)
		db.w[wordIndex(main_a[a])].related.push(msg_b[0]);
}
...
function pickFirstWord(msg){
	...
	if(possible.length < 1)
		return main_words[r(main_words)];
...
function swearFilter(str){
	if(typeof str != String)
		str = str.toString();
	return str.replace(/fuck|shit|damn|porn|dick|ass|slut|cunt|bitch/ig, "****");
}
